...
static void BM_load_datasets(benchmark::State &state) {
  // Setup
  DataSize size = (state.range(0) == 0 ? DataSize::XS : DataSize::S);
...
// Add to benchmarks
BENCHMARK(BM_load_datasets)->Name("LoadDatasets")->Arg(DataSize::XS)->Arg(DataSize::S)->Setup(DoSetup);
BENCHMARK(BM_parse_datasets)->Name("ParseDatasets")->Arg(DataSize::XS)->Arg(DataSize::S)->Setup(DoSetup);
